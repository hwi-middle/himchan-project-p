...
*/
// Sets default values
APPCircuitPuzzleWidgetActor::APPCircuitPuzzleWidgetActor()
{
...
void APPCircuitPuzzleWidgetActor::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void APPCircuitPuzzleWidgetActor::Tick(float DeltaTime)
{
...
void APPCircuitPuzzleWidgetActor::RotatePressedButton(UButton* Button, ECircuitNum ButtonNum)
{
	...
	RotateButton = Button;
	RotateButtonNum = ButtonNum;
	StartRotation = RotateButton->GetRenderTransformAngle();
	CurrentRotation = StartRotation;
	GetWorldTimerManager().SetTimerForNextTick(this, &APPCircuitPuzzleWidgetActor::RotateAction);
}
...
void APPCircuitPuzzleWidgetActor::RotateAction()
{
	...
	RotateButton->SetRenderTransformAngle(CurrentRotation);
	if(CurrentRotation > StartRotation + 90 || !CurrentRotation)
	{
		RotateButton->SetRenderTransformAngle(StartRotation + 90);
		bIsRotationOnGoing = false;
		CheckCurrentButtonCorrect();
		return;
	}
	GetWorldTimerManager().SetTimerForNextTick(this, &APPCircuitPuzzleWidgetActor::RotateAction);
}

void APPCircuitPuzzleWidgetActor::CheckCurrentButtonCorrect()
{
	switch (RotateButtonNum)
	{
...
ECircuitDirection APPCircuitPuzzleWidgetActor::SetNextDirection(ECircuitDirection Direction)
{
	switch (Direction)
	{
